...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $G_{ijkl} \gets (\phi_i \phi_j | \phi_k \phi_l)$ \Comment{Compute AO two-electron integrals using PySCF}
        \State $\Gamma_{\mu \nu \lambda \sigma} \gets \sum_{ijkl} C_{i \mu} C_{j \nu} C_{k \lambda} C_{l \sigma} \cdot G_{ijkl}$ \Comment{Compute MO two-electron integrals}
